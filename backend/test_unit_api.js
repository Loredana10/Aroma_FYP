...
// db.js creates a new Pool() at the top level when it is first require()d.
// By mocking the 'pg' module here — before any require() runs — the Pool
// constructor returns our mock client instead of trying to connect to Postgres.
...
// ─── HELPER: get the mock pool instance ───────────────────────────────────────
// After jest.mock('pg') above, every require('pg') returns our mock.
// db.js does  const pool = new Pool(...)  — that pool is our mockPool above.
// We access it through pg so we can reset mocks between tests.

function getPool() {
  const { Pool } = require('pg');
  return Pool.mock.results[0]?.value ?? Pool();
}

// ─── BUILD APP ────────────────────────────────────────────────────────────────

function buildApp(routerPath) {
  // Clear the route from require cache so each describe gets a fresh load
